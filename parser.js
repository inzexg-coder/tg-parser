...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function analyzeChat(data) {
        // Try to find messages array in various possible structures
        let messages = [];
        let chatTitle = 'Unknown Chat';
        let chatType = 'unknown';

        // Common Telegram export structures
        if (data.chats && Array.isArray(data.chats.list)) {
            // Assume first chat is the one we want
            ...
            // Fallback: try to find any array of objects with 'text' or 'message'
            const findMessages = (obj) => {
                if (Array.isArray(obj)) {
                    // Check if first element looks like a message
                    if (obj.length > 0 && typeof obj[0] === 'object') {
                        const first = obj[0];
                        if (first.text || first.message || first.content) {
                            return obj;
                        }
                    }
        ...
        // Filter out system messages and service messages if needed
        messages = messages.filter(m => 
            m.type !== 'service' && 
            (m.text || m.message || m.content) && 
            !(m.actor && m.actor === 'chat_create')
        );
        ...
        const unifiedMessages = messages.map(msg => {
            ...
            const date = timestamp ? new Date(timestamp * 1000) : new Date(); // Assume Unix seconds if needed
    ...
    function getResponseTimes(messages) {
        // Simplified: time between consecutive messages from different senders
        const times = [];
        for (let i = 1; i < messages.length; i++) {
            if (messages[i].sender !== messages[i-1].sender) {
                const diff = messages[i].timestamp - messages[i-1].timestamp;
                if (diff > 0 && diff < 86400000) { // Less than a day to avoid outliers
    ...
    function createCharts(stats, messages) {
        ...
        // Chart 1: Messages per day (line chart)
        ...
        // Chart 2: Messages per hour (bar chart)
        ...
        // Chart 3: Messages by day of week (bar chart)
        ...
        // Chart 4: Top senders (horizontal bar chart)
        ...
        const topSenders = stats.topSenders.slice(0, 8); // Top 8 for readability
        ...
        // Chart 5: Media type distribution (pie chart)
        ...
        // Chart 6: Response time distribution (box plot approximation with histogram)
        ...
        if (responseTimes.length > 0) {
            // Create histogram bins (0-60s, 60-300s, 300-600s, 600-1800s, 1800+)
